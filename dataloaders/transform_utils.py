...
# import SLAug.dataloaders.image_transforms as myit
...
class transform_with_label(object):
    ...
    def __call__(self, comp, c_label, c_img, nclass, is_train, use_onehot = False):
        ...
        if is_train is True:
            ...
            _h_label = np.float32(np.arange( nclass ) == (_label[..., None]) )
            comp = np.concatenate( [comp[...,  :c_img ], _h_label], -1 )
            comp = self.geometric_tfx(comp)  # 이거 주석하면 geometric augmentation 동작 x
            # round one_hot labels to 0 or 1
            t_label_h = comp[..., c_img : ]
            t_label_h = np.rint(t_label_h)
            t_img = comp[..., 0 : c_img ]
        ...
        # intensity transform
        t_img = self.intensity_tfx(t_img)  # 이거 주석하면 intensity augmentation 동작 x
