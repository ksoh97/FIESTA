...
import glob
import numpy as np
# import SLAug.dataloaders.niftiio as nio
# import SLAug.dataloaders.transform_utils as trans
...
hostname = platform.node()  # 사용하고 있는 GPU 정보 제공

# folder for datasets
BASEDIR = '/home/ksoh/DG_data/abdominal/'
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if self.location_scale is not None:
                img = curr_dict["img"].copy()  # (192, 192, 1)
                lb = curr_dict["lb"].copy()  # (192, 192, 1)
        ...
        if self.tile_z_dim > 1:  # 3-channels image인 경우에 동작
